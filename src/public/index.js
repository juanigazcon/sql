...
const onProductSubmit = (e, form) => {
    ...
    if (obj.title && obj.price && obj.url) {
...
const tablaProductos = (products) =>{
    ...
    if(products.length>0){
       ...
       products.forEach((item)=>{
        ...
        `
                <tr>
                    <td><p style="text-align:center; font-weight:bold">${item.title}</p></td>
                    <td><p style="text-align:center; font-weight:bold">$${item.price}</p></td>
                    <td><p style="text-align:center; font-weight:bold"><img src="${item.url}" alt="${item.title}" style='width:40%'></p></td>
                </tr>
        `
        })
